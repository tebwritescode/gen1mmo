# Mod index submission (bryanthaboi/gen1recomp-mod-index)

Everything the index PR needs, ready to paste. Submit via the helper at
https://bryanthaboi.github.io/gen1recomp-mod-index/ (it produces the PR).

| Field | Value |
|---|---|
| Title | Gen1MMO |
| Author | tebwritescode |
| Mod ID | `gen1mmo` (matches manifest.json) |
| Version | current release tag, no leading `v` |
...
Release asset naming the launcher's updater requires: `gen1mmo-<version>.zip`
with the mod files at the ARCHIVE ROOT (manifest.json at top level, not
nested in a folder).
...
filtered and never stored, transport is end-to-end encrypted against a
